...
module.exports = function (type, length, blockSize, cb) {
    try {
        ...
        return cb ?
            withCallback(type, length, blockSize, cb) :
            withPromise(type, length, blockSize);
    } catch (error) {
        return cb ?
            cb(`` + error) :
...
function withCallback(type, length, blockSize, cb) {
    fetchNumbers((data) => cb(null, data));
}

function withPromise(type, length, blockSize) {
